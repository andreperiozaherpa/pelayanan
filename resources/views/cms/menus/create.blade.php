...
@section('title', 'Tambah Menu Navigasi Baru')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Tambah Menu Navigasi</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Buat item menu navigasi baru untuk mengarahkan pengguna ke halaman statis atau tautan eksternal.</p>
        </div>
    </div>
    ...
    <div class="premium-card bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-black/[0.03] dark:border-white/[0.03] shadow-sm">
        <div class="p-8 sm:p-10">
            <form action="{{ route('cms-menus.store') }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="target" id="target" required
                            class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white">
                            <option value="_self" {{ old('target', '_self') == '_self' ? 'selected' : '' }}>Tab Sama (_self)</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="link_type"
                            class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white">
                            <option value="page" {{ old('cms_page_id') ? 'selected' : '' }}>Halaman Statis CMS</option>
                            <option value="custom" {{ !old('cms_page_id') && old('url') ? 'selected' : '' }}>Tautan / URL Kustom</option>
                        </select>
                    </div>
                    ...
                    <div id="page_wrapper" class="space-y-2 col-span-2">
                        ...
                        <select name="cms_page_id" id="cms_page_id"
                            ...
                            @foreach($pages as $page)
                                <option value="{{ $page->id }}" {{ old('cms_page_id') == $page->id ? 'selected' : '' }}>
                    ...
                    <div class="space-y-2 col-span-2">
                        ...
                        <textarea name="description" rows="2"
                            class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white"
                            placeholder="Contoh: Menampilkan daftar layanan administrasi warga...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-[10px] text-rose-500 font-bold mt-1 uppercase tracking-tight">{{ $message }}</p>
                        @enderror
                    </div>
