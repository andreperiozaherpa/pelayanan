    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 font-medium tracking-tight">Kelola isi halaman statis seperti About Us,
                    Services, atau T&C.</p>
            </div>
        ...
        <!-- Table -->
        <div
            class="premium-card bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-black/[0.03] dark:border-white/[0.03] shadow-sm">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-black/[0.02] dark:border-white/[0.02]">
                            <th class="py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Judul
                                Halaman</th>
                            <th class="py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Slug</th>
                            <th
                                class="py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                                Status</th>
                            <th
                    ...
                    <tbody class="divide-y divide-black/[0.02] dark:divide-white/[0.02]">
                        @forelse($pages as $page)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition group">
                                <td class="py-4 px-6">
                                    <span
                                        class="text-[11px] font-black text-slate-800 dark:text-white uppercase tracking-wider group-hover:text-primary-acorn transition">
                                        {{ $page->title }}
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    @if ($page->menu)
                                        <a href="{{ $page->menu->url }}" target="_blank"
                                            class="hover:underline">
                                            <code
                                                class="text-[10px] font-mono text-slate-500 hover:text-primary-acorn bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
                                                {{ $page->menu->url }}
                                            </code>
                                        </a>
                                    @else
                                        <span class="text-[9px] font-black text-amber-500 bg-amber-500/10 border border-amber-500/10 px-2.5 py-1 rounded-lg uppercase tracking-widest">
                                            Belum Ditautkan
                                        </span>
                                    @endif
                                </td>
                                <td class="py-4 px-6 text-center">
                                    @if ($page->is_active)
                                        <span
                                            class="px-2.5 py-1 rounded-lg text-[9px] font-black bg-emerald-500/10 text-emerald-500 border border-emerald-500/10 uppercase tracking-widest">
                                            AKTIF
                                        </span>
                                    @else
                                        <span
                                            class="px-2.5 py-1 rounded-lg text-[9px] font-black bg-slate-500/10 text-slate-500 border border-slate-500/10 uppercase tracking-widest">
                                            NONAKTIF
                                        </span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @can('cms.pages.delete')
                                            <form action="{{ route('cms-pages.destroy', $page->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="button"
                                                    class="delete-btn p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-500/5 rounded-lg transition"
                                                    title="Hapus Halaman">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-16 text-center">
                                    <p class="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Tidak ada
                                        halaman ditemukan.</p>
                                </td>
                            </tr>
                        @endforelse
