...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Edit Banner</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui informasi banner slider "{{ $cmsBanner->title }}".</p>
        </div>
    </div>
    ...
    <div class="premium-card bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-black/[0.03] dark:border-white/[0.03] shadow-sm">
        <div class="p-8 sm:p-10">
            <form action="{{ route('cms-banners.update', $cmsBanner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="title" value="{{ old('title', $cmsBanner->title) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="subtitle" rows="3"
                                class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white"
                                placeholder="Teks penjelasan banner slider...">{{ old('subtitle', $cmsBanner->subtitle) }}</textarea>
                            @error('subtitle')
                                <p class="text-[10px] text-rose-500 font-bold mt-1 uppercase tracking-tight">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="col-span-2 space-y-2">
                                ...
                                <input type="url" name="link_url" value="{{ old('link_url', $cmsBanner->link_url) }}"
                                    class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white"
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" name="order" value="{{ old('order', $cmsBanner->order) }}" required min="0"
                                    class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white" />
                                @error('order')
                                    <p class="text-[10px] text-rose-500 font-bold mt-1 uppercase tracking-tight">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            @include('cms.partials.dropzone-upload', [
                                'id' => 'image',
                                'name' => 'image',
                                'label' => 'Ganti File Gambar',
                                'value' => old('image', $cmsBanner->image_path),
                        ...
                        <label class="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900/30 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl hover:border-primary-acorn/50 transition-all cursor-pointer group">
                            <div class="relative inline-flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $cmsBanner->is_active) ? 'checked' : '' }}
