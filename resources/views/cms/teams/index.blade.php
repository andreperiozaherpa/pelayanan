...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Struktur Organisasi</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Kelola daftar pimpinan, pejabat, dan staf organisasi.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-black/[0.02] dark:border-white/[0.02]">
                        <th class="py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Foto & Nama</th>
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition group">
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0 border border-black/[0.03] dark:border-white/[0.03]">
                                        @if($member->image)
                                            <img src="{{ asset('storage/' . $member->image) }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" alt="{{ $member->name }}">
                                        @else
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    @can('cms.teams.edit')
                                    <a href="{{ route('cms-teams.edit', $member->id) }}" class="p-2 text-slate-400 hover:text-primary-acorn hover:bg-primary-acorn/5 rounded-lg transition" title="Edit Pejabat/Staf">
                                    ...
                                    <form action="{{ route('cms-teams.destroy', $member->id) }}" method="POST" class="delete-form inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="delete-btn p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-500/5 rounded-lg transition" title="Hapus Pejabat/Staf">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <p class="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Tidak ada data struktur organisasi ditemukan.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Apakah Anda yakin ingin menghapus data pejabat/staf ini?",
                    icon: 'warning',
