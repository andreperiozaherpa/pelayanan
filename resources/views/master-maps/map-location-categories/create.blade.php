...
@section('title', 'Tambah Kategori Lokasi')

@section('content')
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Tambah Kategori Lokasi</h1>
        <p class="text-xs text-slate-500 font-medium tracking-tight">Definisikan kategori titik lokasi baru beserta ikon penandanya.</p>
    </div>

    <div class="premium-card p-6 md:p-8 max-w-xl">
        <form action="{{ route('map-location-categories.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Kantor Pemerintah, Rumah Ibadah" required
            ...
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon', 'lucide:map-pin') }}" placeholder="Contoh: lucide:church, mdi:hospital" required
                    class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
