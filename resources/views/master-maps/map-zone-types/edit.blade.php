...
@section('title', 'Ubah Tipe Zonasi')

@section('content')
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Ubah Tipe Zonasi</h1>
        <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui informasi atribut klasifikasi tipe zonasi.</p>
    </div>

    <div class="premium-card p-6 md:p-8 max-w-xl">
        <form action="{{ route('map-zone-types.update', $mapZoneType->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $mapZoneType->name) }}" required
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <input type="color" name="color" value="{{ old('color', $mapZoneType->color) }}"
            ...
            <div>
                ...
                <textarea name="description" placeholder="Penjelasan singkat mengenai tipe zonasi ini..." rows="3"
                    class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400">{{ old('description', $mapZoneType->description) }}</textarea>
