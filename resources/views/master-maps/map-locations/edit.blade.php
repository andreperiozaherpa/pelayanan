...
@section('title', 'Ubah Titik Lokasi')

@section('content')
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Ubah Titik Lokasi (POI)</h1>
        <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui data spasial, kategori, foto, dan area restriksi titik lokasi.</p>
    </div>

    <div class="premium-card p-6 md:p-8">
        <form action="{{ route('map-locations.update', $mapLocation->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $mapLocation->name) }}" required
                    ...
                    <div>
                        ...
                        <select name="category_id" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" data-color="{{ $cat->color }}" data-icon="{{ $cat->icon }}" {{ $mapLocation->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="region_id"
                            ...
                            @foreach($regions as $reg)
                                <option value="{{ $reg->id }}" {{ $mapLocation->region_id == $reg->id ? 'selected' : '' }}>{{ $reg->name }} ({{ $reg->level }})</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.00000001" name="latitude" id="latitude" value="{{ old('latitude', $mapLocation->latitude) }}" required
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
                    ...
                    <div>
                        ...
                        <input type="number" step="0.00000001" name="longitude" id="longitude" value="{{ old('longitude', $mapLocation->longitude) }}" required
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
                ...
                <div>
                    ...
                    <input type="text" name="address" value="{{ old('address', $mapLocation->address) }}" placeholder="Contoh: Jl. Diponegoro No. 12, Panaragan Jaya"
                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
                ...
                <div>
                    ...
                    <textarea name="description" placeholder="Informasi tambahan mengenai fasilitas..." rows="2"
                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400">{{ old('description', $mapLocation->description) }}</textarea>
                ...
                <div>
                    ...
                    @if($mapLocation->photo)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $mapLocation->photo) }}" alt="Foto" class="h-24 w-36 object-cover rounded-xl border border-black/10 dark:border-white/10">
                        </div>
                    @endif
                ...
                <div x-data="{ hasRestriction: {{ $restriction ? 'true' : 'false' }}, restrictionMode: '{{ $restriction ? 'manual' : 'radius' }}' }" @set-restriction.window="hasRestriction = $event.detail.hasRestriction; restrictionMode = $event.detail.restrictionMode">
                    ...
                    <div x-show="hasRestriction" class="mt-4 bg-rose-500/5 border border-rose-500/10 p-5 rounded-2xl space-y-4">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 gap-2.5">
                                @php
                                    $currentActivities = $restriction ? $restriction->restricted_activities : [];
                                @endphp
                                @foreach(['Aktivitas Industri', 'Pembangunan Tempat Hiburan', 'Perdagangan & Jasa Skala Besar', 'Pembangunan Rumah Ibadah Lain', 'Peternakan Skala Besar'] as $act)
                                    <label class="flex items-center gap-2.5 cursor-pointer">
                                        <input type="checkbox" name="restricted_activities[]" value="{{ $act }}"
                                            {{ in_array($act, $currentActivities) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <textarea name="restriction_notes" placeholder="Contoh: Dilarang mendirikan usaha industri dalam radius 500m dari rumah ibadah..." rows="2"
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400">{{ old('restriction_notes', $restriction->notes ?? '') }}</textarea>
                        </div>

                        {{-- Hidden field for restriction GeoJSON polygon --}}
                        <textarea name="restriction_geojson" id="restriction_geojson" class="hidden">{{ $restriction ? json_encode($restriction->geojson) : '' }}</textarea>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const map = L.map('map-draw', {
            zoomControl: false
        }).setView([parseFloat(latInput.value), parseFloat(lngInput.value)], 13);
        ...
        const existingPhoto = @json($mapLocation->photo ? asset('storage/' . $mapLocation->photo) : null);
        ...
        poiMarker.on('dragend', function(e) {
            ...
            if (typeof updateRadiusRestriction === 'function') {
                updateRadiusRestriction();
            }
        });
        ...
        // Load existing restriction geometry if exists
        if (restrictionGeojson.value.trim() !== '') {
            try {
                const geom = JSON.parse(restrictionGeojson.value);
                ...
                // Zoom to restriction bounds
                map.fitBounds(drawnItems.getBounds(), { padding: [40, 40] });
            } catch (e) {
                console.error("Failed to parse restriction GeoJSON:", e);
            }
        }
