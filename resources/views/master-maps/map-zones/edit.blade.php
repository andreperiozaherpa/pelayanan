...
@section('title', 'Ubah Zona Fungsi')

@section('content')
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Ubah Zona Fungsi</h1>
        <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui nama, klasifikasi tipe kawasan, wilayah, atau batas poligon zona fungsional.</p>
    </div>

    <div class="premium-card p-6 md:p-8">
        <form action="{{ route('map-zones.update', $mapZone->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Nama Kawasan Zona</label>
                    <input type="text" name="name" value="{{ old('name', $mapZone->name) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5 block">Tipe Klasifikasi Kawasan</label>
                        <select name="zone_type_id" id="zone_type_id" required
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" data-color="{{ $type->color }}" {{ $mapZone->zone_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                        @error('zone_type_id') <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="region_id"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all text-slate-700 dark:text-slate-300">
                            <option value="">— Kabupaten (Seluruh Tubaba) —</option>
                            @foreach($regions as $reg)
                                <option value="{{ $reg->id }}" {{ $mapZone->region_id == $reg->id ? 'selected' : '' }}>{{ $reg->name }} ({{ $reg->level }})</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" placeholder="Tuliskan keterangan detail mengenai regulasi kawasan atau peruntukannya..." rows="4"
                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400">{{ old('description', $mapZone->description) }}</textarea>
                    @error('description') <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                </div>

                {{-- Hidden input for GeoJSON data --}}
                <textarea name="geojson" id="geojson" class="hidden">{{ json_encode($mapZone->geojson) }}</textarea>
                ...
                <div class="flex items-center gap-3 pt-4">
                    ...
                    <a href="{{ route('map-zones.index') }}" class="px-6 py-3.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
                        Batal
                    </a>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3">
                <div>
                    <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1 block">Gambar Batas Poligon Zona</label>
                    <p class="text-[10px] text-slate-400 font-medium">Ubah atau buat ulang batas poligon kawasan menggunakan alat gambar di sebelah kanan.</p>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const typeSelect = document.getElementById('zone_type_id');

        function getActiveColor() {
            const selectedOption = typeSelect.options[typeSelect.selectedIndex];
            return selectedOption ? selectedOption.getAttribute('data-color') : '#3b82f6';
        }
        ...
        // Load existing geometry if exists
        const geojsonArea = document.getElementById('geojson');
        const drawStatus = document.getElementById('draw-status');
        
        if (geojsonArea.value.trim() !== '') {
            try {
                const geom = JSON.parse(geojsonArea.value);
                ...
                console.error("Failed to parse existing GeoJSON:", e);
            }
        }

        // Add Leaflet Draw controls
        let drawControl = createDrawControl(getActiveColor());
        ...
        // Update color dynamically when type changes
        typeSelect.addEventListener('change', () => {
            ...
            // Re-create drawing controls with updated color
