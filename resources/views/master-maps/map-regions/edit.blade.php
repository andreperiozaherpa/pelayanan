...
@section('title', 'Ubah Batas Wilayah')

@section('content')
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Ubah Batas Wilayah</h1>
        <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui informasi administratif dan batas poligon wilayah.</p>
    </div>

    <div class="premium-card p-6 md:p-8">
        <form action="{{ route('map-regions.update', $mapRegion->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $mapRegion->name) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $mapRegion->code) }}"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
                    ...
                    <div>
                        ...
                        <select name="level" required
                            ...
                            <option value="desa" {{ $mapRegion->level === 'desa' ? 'selected' : '' }}>Tiyuh/Desa</option>
                            <option value="kecamatan" {{ $mapRegion->level === 'kecamatan' ? 'selected' : '' }}>Kecamatan</option>
                            <option value="kabupaten" {{ $mapRegion->level === 'kabupaten' ? 'selected' : '' }}>Kabupaten</option>
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="parent_id"
                            ...
                            @foreach($parents as $parent)
                                <option value="{{ $parent->id }}" {{ $mapRegion->parent_id == $parent->id ? 'selected' : '' }}>{{ $parent->name }} ({{ $parent->level }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="color" name="color" value="{{ old('color', $mapRegion->color ?? '#3b82f6') }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.0001" name="area_km2" value="{{ old('area_km2', $mapRegion->area_km2) }}" placeholder="Contoh: 14.52"
                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-black/[0.03] dark:border-white/[0.03] rounded-xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white placeholder-slate-400" />
                    @error('area_km2') <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                </div>

                {{-- Hidden input for GeoJSON data --}}
                <textarea name="geojson" id="geojson" class="hidden">{{ $mapRegion->geojson ? json_encode($mapRegion->geojson) : '' }}</textarea>
            ...
            <div class="flex flex-col gap-3">
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 font-medium">Ubah atau buat ulang batas poligon spasial menggunakan alat gambar di sebelah kanan.</p>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Update polygon color style dynamically when color picker changes
        colorInput.addEventListener('input', (e) => {
