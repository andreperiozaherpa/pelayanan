...
@section('title', 'Buat Pelayanan MPP')
...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Buat Pelayanan Baru</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Konfigurasi pelayanan publik MPP dengan formulir dinamis.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('mpp-services.store') }}" enctype="multipart/form-data" class="space-y-6" x-data="mppServiceForm()">
        @csrf
            ...
            <div class="p-8 sm:p-12 space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <select name="opd_id" x-model="opdId" @change="geraiId = ''"
                        ...
                        @foreach($opds as $opd)
                            <option value="{{ $opd->id }}" {{ old('opd_id') == $opd->id || request('opd_id') == $opd->id ? 'selected' : '' }}>
                                {{ $opd->code }} - {{ $opd->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3"
                        ...
                        placeholder="Deskripsi pelayanan...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-[9px] font-bold text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@push('scripts')
<script>
    function mppServiceForm() {
        return {
            opds: @json($opds),
            opdId: '{{ old('opd_id', request('opd_id')) }}',
            geraiId: '{{ old('gerai_id') }}',
            fields: [],
            addField() {
                this.fields.push({
                    label: '',
