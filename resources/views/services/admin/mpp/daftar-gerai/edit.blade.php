...
<div class="max-w-2xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Edit Gerai</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Perbarui informasi gerai {{ $gerai->name }}.</p>
        </div>
    </div>

    <div class="premium-card">
        <div class="p-8 sm:p-12">
            <form method="POST" action="{{ route('gerais.update', $gerai) }}" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <select name="opd_id"
                        ...
                        @foreach($opds as $opd)
                            <option value="{{ $opd->id }}" {{ old('opd_id', $gerai->opd_id) == $opd->id ? 'selected' : '' }}>
                                {{ $opd->code }} - {{ $opd->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="code" value="{{ old('code', $gerai->code) }}"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $gerai->name) }}"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="location" value="{{ old('location', $gerai->location) }}"
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-[9px] font-bold text-slate-400">Kosongkan/kosongkan preview jika tidak mengubah logo.</p>
                    @error('logo')
                        <p class="text-[9px] font-bold text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $gerai->is_active ? 'checked' : '' }}
