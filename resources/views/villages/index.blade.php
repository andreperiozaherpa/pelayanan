...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
            <a href="{{ route('villages.create') }}" class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
    ...
    <!-- Search & Filters -->
    <div class="mb-6">
        <form action="{{ route('villages.index') }}" method="GET" class="relative group">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-100/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700/50">
                        ...
                        <th class="py-4 px-6 text-xs font-black text-slate-500 uppercase tracking-widest text-center">Kode Desa</th>
                    ...
                    @forelse($villages as $village)
                        <tr class="hover:bg-white/60 dark:hover:bg-slate-800/60 transition-colors group">
                            ...
                            <td class="py-4 px-6 text-center whitespace-nowrap">
                                <span class="px-3 py-1 rounded-full text-[10px] font-black bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/5 tracking-wider">
                                    {{ $village->code }}
                                </span>
                            </td>
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('villages.edit', $village->id) }}" class="p-2 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl transition border border-slate-200 dark:border-white/5" title="Edit Desa">
                                    ...
                                    <form action="{{ route('villages.destroy', $village->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus desa ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white rounded-xl transition border border-rose-500/20" title="Delete Desa">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
