...
<body class="font-sans antialiased bg-[#0f172a] text-slate-200 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            <div
                class="inline-flex w-16 h-16 bg-emerald-500 rounded-2xl items-center justify-center shadow-2xl shadow-emerald-500/20 mb-4">
            ...
            <h1 class="text-2xl font-bold text-white tracking-tight">Sistem Verifikasi<br>Pelayanan Dokumen</h1>
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-[2.5rem] p-10 shadow-2xl">
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    <label for="email"
                        class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Alamat
                        Email</label>
                    <div class="relative">
                        <div
                    ...
                    @error('email')
                        <p class="text-rose-500 text-[10px] font-bold uppercase tracking-widest mt-1 ml-1">
                            {{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    <label for="password"
                        class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Kata Sandi</label>
                    <div class="relative">
                        <div
                            class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex items-center justify-between px-1">
                    <label class="flex items-center group cursor-pointer">
                        <input type="checkbox" name="remember"
                            class="w-4 h-4 rounded border-slate-700 bg-slate-900 text-emerald-500 focus:ring-emerald-500/50 transition">
                        <span class="ml-2 text-xs text-slate-400 group-hover:text-slate-300">Ingat saya</span>
                    </label>
                    <a href="#" class="text-xs text-emerald-500 font-bold hover:text-emerald-400 transition">Lupa
                        sandi?</a>
                </div>

                <button type="submit"
                    class="w-full py-5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl transition transform hover:-translate-y-0.5 active:scale-95 shadow-xl shadow-emerald-600/20">
                    Masuk ke Dashboard
                </button>
            </form>
        </div>
