                ...
                <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between mb-2">
                            <label
                                class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Daftar
                                Izin Akses (Permissions)</label>
                            <button type="button"
                                onclick="const checks = document.querySelectorAll('input[type=checkbox]'); const allChecked = Array.from(checks).every(c => c.checked); checks.forEach(c => c.checked = !allChecked)"
