...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Tambah OPD</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Input informasi Organisasi Perangkat Daerah baru.</p>
        </div>
    </div>

    <div class="premium-card overflow-hidden">
        <div class="p-8 sm:p-10">
            <form action="{{ route('opds.store') }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900/50 border border-black/[0.03] dark:border-white/[0.03] rounded-2xl focus:ring-2 focus:ring-primary-acorn/20 focus:border-primary-acorn text-[11px] font-bold transition-all dark:text-white @error('description') border-red-500 @enderror"
                            placeholder="Masukkan deskripsi singkat mengenai tugas pokok dan fungsi OPD...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-[10px] font-bold mt-1 uppercase tracking-tight">{{ $message }}</p>
                        @enderror
                    </div>
