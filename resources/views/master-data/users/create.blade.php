...
@php
    $initialRoleId = old('role_id', '');
    $initialRole = $initialRoleId ? $roles->firstWhere('id', $initialRoleId) : null;
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Tambah User</h1>
            <p class="text-xs text-slate-500 font-medium tracking-tight">Daftarkan pengguna baru dengan hak akses yang spesifik.</p>
        </div>
    </div>
    ...
    <div class="premium-card overflow-hidden">
        <div class="p-8 sm:p-10">
            <form action="{{ route('users.store') }}" method="POST" class="space-y-8" x-data="{
                role: '{{ $initialRoleId }}',
                tipe: '{{ $initialTipe }}',
                roleSlug() {
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="desa_id" id="desa" class="user-select2" data-placeholder="PILIH DESA...">
                                ...
                                @foreach($villages as $v)
                                    <option value="{{ $v->id }}" data-district="{{ $v->district_id }}" {{ old('desa_id') == $v->id ? 'selected' : '' }}>
                                        {{ strtoupper($v->name) }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- Password -->
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Password</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Konfirmasi Password</label>
