...
<body class="min-h-screen antialiased flex flex-col justify-between">
    ...
    <nav x-data="{ scrolled: {{ Route::is('landing.index') ? 'false' : 'true' }}, open: false }"
        ...
        :class="scrolled ? 'bg-[#6d272e]/95 dark:bg-[#5d1e25]/95 backdrop-blur-md shadow-lg' :
            'bg-[#6d272e] border-b border-white/10'"
                    ...
                    @if ($menu->children->isNotEmpty())
                        @if ($menu->title === 'Pelayanan')
                            ...
                            <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false"
                                class="relative">
                                <button
                                    class="font-semibold text-sm text-white/80 hover:text-white transition-colors duration-200 flex items-center gap-1 py-4">
                                    {{ $menu->title }}
                                        ...
                                        <a href="{{ $child->url }}" target="{{ $child->target }}"
                                            ...
                                            @if ($child->icon)
                                                <iconify-icon icon="{{ $child->icon }}"
                                                    class="text-xl text-[#f4edd8] mt-1"></iconify-icon>
                                            @else
                                                <div
                                                    class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-[#f4edd8] flex-shrink-0">
                                                    <span
                                                        class="text-xs font-bold">{{ substr($child->title, 0, 2) }}</span>
                                                </div>
                                            @endif
                                            <div>
                                                <div
                                                    class="text-sm font-semibold text-white group-hover/item:text-[#f4edd8] transition-colors">
                                                    {{ $child->title }}
                                                </div>
                ...
                @if ($menu->children->isNotEmpty())
                    <div x-data="{ subOpen: false }" class="py-1">
                        <button @click="subOpen = !subOpen"
                            ...
                            <svg class="w-4 h-4 transition-transform" :class="subOpen ? 'rotate-180' : ''"
