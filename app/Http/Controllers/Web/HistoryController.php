...
class HistoryController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $audits = AuditLog::with('user')
            ...
            ->map(function ($log) {
                $verbs = [
                    ...
                    'APPROVE_SERVICE' => 'Menyetujui Layanan',
                    'REJECT_SERVICE' => 'Menolak Layanan',
                    'AUTO_EXPIRE_SERVICE' => 'Layanan Kedaluwarsa (SLA)',
                ...
                } elseif (($log->target_table === 'citizens' || $log->target_table === 'service_requests') && $log->target_id) {
                    $targetId = $log->target_id;
                    if ($log->target_table === 'service_requests' && is_array($log->new_value)) {
                        $targetId = $log->new_value['citizen_nik'] ?? $log->target_id;
                    }
                    $targetName = Cache::remember("citizen_name_{$targetId}", 3600, function () use ($targetId) {
                        return Citizen::where('nik', $targetId)->value('nama_lengkap');
                    });
                }
                ...
                return [
                    ...
                    'details' => match ($log->action) {
                        'REPORT_SERVICE', 'APPROVE_SERVICE' => is_array($log->new_value) ? ($log->new_value['service_type'] ?? '') : '',
                        'REJECT_SERVICE', 'AUTO_EXPIRE_SERVICE' => is_array($log->new_value) ? ($log->new_value['reason'] ?? '') : '',
                        default => '',
                    },
                ];
            });
        ...
        return view('services.admin.histories.index', [
            'logs' => $groupedLogs,
            'paginator' => $logs,
            'search' => $search,
            'perPage' => $perPage,
