...
import {
  GeraiGetWaitingList,
  GeraiGetCurrentCalling,
  GeraiPanggilBerikutnya,
  GeraiSelesai,
  GeraiPanggilUlang,
  getErrorMessage,
} from '../services/api'
...
export default function Gerai() {
  const { petugas } = useApp()
  const { showToast } = useToast()
  const geraiID = petugas?.counter_id
  ...
  const subText = useMemo(() => {
    ...
    return svc?.instansi?.nama || petugas?.counter_nama || 'Instansi'
  }, [calling, waiting, petugas])

  const loadData = useCallback(async () => {
    if (!geraiID) return
    try {
      const [w, c] = await Promise.all([
        GeraiGetWaitingList(geraiID),
        GeraiGetCurrentCalling(geraiID),
      ])
      setWaiting(w)
      setCalling(c)
      setPage(1)
    } catch (err) {
      console.error('Gerai load error:', err)
    } finally {
      setLoading(false)
    }
  }, [geraiID])
  ...
  useEffect(() => {
    SetStaffWindow().catch(err => console.error('SetStaffWindow Gerai:', err))
  }, [])
  ...
  useWebSocket((msg) => {
    if (msg.channel === 'gerai' || msg.channel === 'all') loadData()
  })
  ...
  async function handlePanggil() {
    ...
    try {
      const ticket = await GeraiPanggilBerikutnya(geraiID)
      setCalling(ticket)
  ...
  async function handleSelesai() {
    ...
    setActionLoading(true)
    try {
      await GeraiSelesai(calling.id, catatan)
      setCalling(null)
      setCatatan('')
      setTargetTicket(null)
      setShowSelesaiModal(false)
      showToast(`Layanan nomor ${calling.nomor_antrian} selesai.`, 'success')
    } catch (err) {
      showToast(getErrorMessage(err, 'Gagal menyelesaikan layanan'), 'error')
    } finally {
      setActionLoading(false)
    }
  }
  ...
  async function handlePanggilUlang() {
    ...
    setActionLoading(true)
    try {
      const ticket = await GeraiPanggilUlang(calling.id)
      setCalling(ticket)
  ...
  return (
    <div className="page">
      ...
      <div className="fo-page">
        ...
        <header className="fo-header">
          <div className="fo-header-greet">
            <h1 className="fo-header-greet-title">{getGreeting()}, Gerai</h1>
            <span className="fo-header-greet-sub">{petugas?.counter_nama || petugas?.nama || petugas?.name || ''}</span>
          </div>
          <h2 className="fo-header-title">Antrian Gerai</h2>
          <UserMenu />
        </header>
        ...
        <main className="fo-content">
          ...
          {/* ── Kartu nomor yang sedang dilayani ── */}
          {calling && (
            <div className="fo-calling">
              <div className="fo-calling-info">
                <p className="fo-calling-label">DILAYANI</p>
                <div className="fo-calling-number">{calling.nomor_antrian}</div>
                <p className="fo-calling-service">{calling.service_name}</p>
                <p className="fo-calling-dur">Waktu berjalan: <strong>{formatDuration(calling.gerai_called_at || calling.called_at, now)}</strong></p>
              </div>
              <div className="fo-calling-actions">
                <button className="fo-calling-btn accent" onClick={() => setShowSelesaiModal(true)} disabled={actionLoading}>
                  <CheckIcon /> Selesai
                </button>
      ...
      <GlassModal
        ...
        actions={
          <>
            <GlassButton variant="accent" onClick={handleSelesai} disabled={actionLoading}>
              {actionLoading ? 'Memproses...' : 'Selesaikan'}
            </GlassButton>
            <GlassButton variant="secondary" onClick={() => setShowSelesaiModal(false)}>Batal</GlassButton>
          </>
        }
      >
        <p className="modal-info">Nomor: <strong>{calling?.nomor_antrian}</strong></p>
        <GlassInput textarea placeholder="Catatan / Keterangan (opsional)" value={catatan} onChange={e => setCatatan(e.target.value)} />
      </GlassModal>
